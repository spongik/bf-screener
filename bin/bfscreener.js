...
if (system.args[1] == '/?') {
  ...
  console.log('\t--lng=ru\t\tBooking form language (ru, en-US, en-GB, fr, de, it, uk, kk, cs, zh)');
  console.log('\t--cur\t\t\tBooking form currency (RUB, USD, EUR, AMD, AZN, BYR, CNY, GBP, KGS, KZT, TJS, UAH, CHF, TND, GEL, IDR)');
...
var BF_QUERY_PARAMS = ['language', 'languageRegion', 'currency', 'accommodationMode', 'date', 'nights', 'adults', 'children', 'promoCodePlain', 'roomTypes', 'ratePlans', 'state', 'experiment'];
...
var Config = function() {
  ...
  this.defaults = {
    ...
    roomTypes: '26029',
    ratePlans: '17024',
    unavailableRoomTypes: '26028',
    unavailableRatePlans: '17025',
    promoCodePlain: '123',
  ...
  var globalParams = {
    ...
    language: Utils.getInputVar('lng', this.defaults.language).split('-')[0],
    languageRegion: Utils.getInputVar('lng', this.defaults.language).split('-')[1],
...
var Selectors = {
  ...
  roomRateExpand: '.rate-plan__title.x-title, .x-rplrc__name.x-title',
  roomRatesExpand: '.rate-plan__toggle-align, .x-rplc__toggle',
  roomDescriptionExpand: '.room__name.x-title, .x-rgc__name.x-title',
  roomPriceDetailsButton: '.rate-plan__popover-btn, .x-rplsc__details-tooltip',
  roomPriceDetails: '.x-popover',
  roomInfoButton: '.room__list .x-room-group',
  roomBookButton: '.x-rate-plan-list .rate-plan__book-btn, .x-rplc .x-rplsc__button',
  roomQuantitySelect: '.rate-plan__quantity-select .x-select__match, .x-rplsc__select .x-select__match',
  roomQuantitySelectOption: '.rate-plan__quantity-select .x-select__choice:nth-child(2), .x-rplsc__select .x-select__choice:nth-child(2)',
  roomCancellationRuleButton: '.rate-plan__cancellation_rule._with_text, .x-rplrc__cancellation._with_text',
  roomCancellationRule: '.x-rate-plan__popover',

  roomConstructor: '.rate-plan__price-details:has(.rate-plan__form), .x-rplsc:has(.x-rplsc__item._state_constructor)',
  roomConstructorSelect: '.rate-plan__price-details:has(.rate-plan__multiple-placeholder) .x-select__match, .x-rplsc__item._state_constructor .x-select__match',
  roomConstructorSelectOption1: '.rate-plan__price-details:has(.rate-plan__multiple-placeholder) .x-select__choice:nth-child(2), .x-rplsc__item._state_constructor .x-select__choice:nth-child(2)',
  roomConstructorSelectOption2: '.rate-plan__price-details:has(.rate-plan__multiple-placeholder) .x-select__choice:nth-child(3), .x-rplsc__item._state_constructor .x-select__choice:nth-child(3)',
  roomConstructorExtra: '.rate-plan__extra .x-title',

  roomUnavailable: '.room__list-item:has(.room__availability-calendar), .x-rgc__availability-calendar',
  ...
  noRoomsPage: '.p-no-rooms',

  priceListPage: '.p-price-list',

  roomInfoPage: '.room-info',
  roomInfoRateDetails: '.rate-plan__icon',

  previewPage: '.p-preview',
  previewTransfer: '.x-transfer__container',
  previewTransferExpand: '.x-transfer .x-title',
  previewTransferTimeSelect: '.x-transfer__row .x-transfer__input._type_short:last-child .x-select__match',
  ...
  paymentOfficeButton: '.x-payment-list__info-description-icon',
  paymentOffice: '.x-popover',
  paymentGuest: '.x-guest-info',

  paymentCitizenship: '.x-guest-info__field._type_citizenship .x-select__match',
  ...
  modalContent: '.x-modal .x-modal__content',
  modalClose: '.x-modal__close',
  ...
  selectDropdown: '.x-select._opened .x-select__choices'
};
...
runner.register('Large rooms page with manual accommodation', ['lg', 'manual', 'rooms', 'large-rooms'], {
  nights: 2,
  date: Utils.formatDate(wednesday),
  experiment: ['large-rooms']
}, new ScenarioCallChain()
    .wait(Selectors.roomsListPage)
    .render(outDir)
    .click(Selectors.roomRateExpand)
...
runner.register('Large rooms page with auto accommodation', ['lg', 'auto', 'rooms', 'large-rooms'], {
  nights: 2,
  adults: 1,
  date: Utils.formatDate(wednesday),
  accommodationMode: 'auto',
...
runner.register('Rooms page with passed unavailable room', ['lg', 'auto', 'rooms'], {
  roomTypes: config.defaults.unavailableRoomTypes,
...
runner.register('Rooms page with passed unavailable rate', ['lg', 'auto', 'rooms'], {
  roomTypes: config.defaults.roomTypes,
  ratePlans: config.defaults.unavailableRatePlans,
  date: Utils.formatDate(monday)
}, new ScenarioCallChain()
    .wait(Selectors.searchFilterPage)
    .click(Selectors.searchButton)
...
runner.register('Accommodation mode switched message', ['manual', 'rooms'], {
  nights: 1,
  adults: 10,
  date: Utils.formatDate(wednesday),
  accommodationMode: 'auto'
...
runner.register('Unavailable promo rate plan', ['promo'], {
  nights: 1,
  adults: 1,
  date: Utils.formatDate(tuesday),
  promoCodePlain: config.defaults.promoCodePlain,
...
runner.register('Calendar with custom text', ['calendar'], {
  nights: 1,
  date: Utils.formatDate(tuesday)
}, new ScenarioCallChain()
    .wait(Selectors.noRoomsPage)
    .click(Selectors.roomAvailability)
    .render(outDir, Selectors.searchCalendar)
    .done()
);
...
runner.register('Transfers time selects', ['select'], {
  accommodationMode: 'auto',
  nights: 1,
  adults: 1,
  date: Utils.formatDate(wednesday),
  ...
  languageRegion: 'US'
    ...
    .click(Selectors.roomBookButton)
    .click(Selectors.previewTransferExpand)
    .click(Selectors.previewTransferTimeSelect)
    .render(outDir, Selectors.previewTransfer)
    .done()
);
...
runner.register('Price list with common screen width (1980px)', ['lg', 'price-list'], {
  state: 'price-list'
}, new ScenarioCallChain()
    .wait(Selectors.priceListPage)
    .wait(Selectors.roomAvailability)
    .render(outDir)
    .done()
);

runner.register('Price list with phone screen width (320px)', ['xs', 'price-list'], {
  state: 'price-list',
  size: 'xs'
}, new ScenarioCallChain()
    .wait(Selectors.priceListPage)
